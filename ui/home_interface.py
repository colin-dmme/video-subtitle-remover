...
class HomeInterface(QWidget):
    ...
    preview_loaded_signal = Signal(str, object)  # (path, data_dict)
    def __init__(self, parent=None):
        ...
        # 处理中用户浏览的任务索引（-1 表示未浏览 / 不在处理中）
        self._user_browsing_task_index = -1

        # 区域继承剪贴板：存储 List[(ymin,ymax,xmin,xmax)] 或 None
        self._region_clipboard = None
        ...
        self.select_task_signal.connect(self._handle_processing_auto_select)
        self.preview_loaded_signal.connect(self._on_preview_loaded)
    ...
    def __init_widgets(self):
        ...
        self.video_display_component.preset_saved.connect(
            lambda name: self.append_output(tr['SubtitleExtractorGUI']['RegionPresetSaved'].format(name)))
        self.video_display_component.preset_loaded.connect(
            lambda name: self.append_output(tr['SubtitleExtractorGUI']['RegionPresetLoaded'].format(name)))
        self.video_display_component.preset_deleted.connect(
            lambda name: self.append_output(tr['SubtitleExtractorGUI']['RegionPresetDeleted'].format(name)))
        ...
        self.task_list_component.region_copy_requested.connect(self.on_region_copy)
        self.task_list_component.region_paste_requested.connect(self.on_region_paste)
        self.task_list_component.region_apply_all_requested.connect(self.on_region_apply_all)
        self.task_list_component.region_preset_requested.connect(self.on_region_preset_apply)
        self.task_list_component.region_preset_multi_requested.connect(self.on_region_preset_apply_multi)
        self.task_list_component.region_paste_multi_requested.connect(self.on_region_paste_multi)
        self.task_list_component.region_preset_apply_all_requested.connect(self.on_region_preset_apply_all)
        ...
        self.srt_button = PushButton(tr['SubtitleExtractorGUI']['AddSRT'], self)
        self.srt_button.setIcon(FluentIcon.FOLDER)
        self.srt_button.clicked.connect(self.choose_srt_file)
        button_layout.addWidget(self.srt_button)

        self.clear_srt_button = PushButton(tr['SubtitleExtractorGUI']['ClearSRT'], self)
        self.clear_srt_button.clicked.connect(self.clear_srt_file)
        self.clear_srt_button.setEnabled(False)
        button_layout.addWidget(self.clear_srt_button)
    ...
    def on_task_selected(self, index, file_path):
        ...
        if self.current_processing_task_index >= 0:
            if index != self.current_processing_task_index:
                # 用户浏览其他任务：冻结自动选中
                self._user_browsing_task_index = index
                self.task_list_component.set_auto_select_enabled(False)
            else:
                # 用户点回正在处理的任务：恢复自动选中
                self._user_browsing_task_index = -1
                self.task_list_component.set_auto_select_enabled(True)
        else:
            self._user_browsing_task_index = -1
        ...
        self.update_srt_button_state(index)
    
    @Slot(int)
    def _handle_processing_auto_select(self, index):
        """处理中任务自动选中 — 用户浏览其他任务时保留用户视图"""
        if self._user_browsing_task_index >= 0:
            # 用户正在浏览：仅滚动让处理中任务可见，不切换选中行，不加载视频
            self.task_list_component.scroll_to_row(index)
        else:
            # 正常模式：选中并加载视频
    ...
    def on_task_deleted(self, index):
        ...
        if task:
            ...
            self.update_srt_button_state(-1)

    def on_region_copy(self, index):
        """复制指定任务的区域到内存剪贴板"""
        task = self.task_list_component.get_task(index)
        if not task:
            return
        # 如果正在显示该任务，优先使用当前显示的选区（最新）
        if index == self.task_list_component.get_current_task_index():
            sub_areas = self.video_display_component.get_selection_rects()
        else:
            sub_areas = task.options.get(TaskOptions.SUB_AREAS.value, [])
        if not sub_areas:
            self.append_output(tr['TaskList']['NoRegionToCopy'])
            return
        self._region_clipboard = list(sub_areas)
        self.task_list_component.set_paste_enabled(True)
        self.append_output(tr['TaskList']['RegionCopied'])

    def on_region_paste(self, index):
        """将剪贴板中的区域粘贴到指定任务"""
        if not self._region_clipboard:
            self.append_output(tr['TaskList']['RegionClipboardEmpty'])
            return
        regions = list(self._region_clipboard)
        self.task_list_component.update_task_option(index, TaskOptions.SUB_AREAS, regions)
        # 如果当前正在显示该任务，同步更新视频预览
        if index == self.task_list_component.get_current_task_index():
            self.video_display_component.set_selection_rects(regions)
        task = self.task_list_component.get_task(index)
        self.append_output(tr['TaskList']['RegionPasted'].format(task.name if task else index))

    def on_region_paste_multi(self, rows: list):
        """将剪贴板中的区域粘贴到多个选中任务"""
        if not self._region_clipboard:
            self.append_output(tr['TaskList']['RegionClipboardEmpty'])
            return
        ...
        for i in rows:
            self.task_list_component.update_task_option(i, TaskOptions.SUB_AREAS, list(regions))
            if i == cur_idx:
                self.video_display_component.set_selection_rects(regions)
        self.append_output(tr['TaskList']['PasteRegionToSelected'].format(len(rows)))

    def on_region_apply_all(self, index):
        """将指定任务的区域应用到所有其他任务"""
        task = self.task_list_component.get_task(index)
        if not task:
            return
        ...
        for i, t in enumerate(self.task_list_component.get_all_tasks()):
            if i != index:
                self.task_list_component.update_task_option(i, TaskOptions.SUB_AREAS, list(sub_areas))
                count += 1
        self.append_output(tr['TaskList']['RegionAppliedToAll'].format(count))

    def on_region_preset_apply(self, index, preset_name, areas_str):
        """将指定预设应用到单个任务"""
        rects = self._parse_preset_areas(areas_str)
        if not rects:
            return
        self.task_list_component.update_task_option(index, TaskOptions.SUB_AREAS, rects)
        # 如果当前正在显示该任务，同步更新视频预览
        if index == self.task_list_component.get_current_task_index():
            self.video_display_component.set_selection_rects(rects)
            self.video_display_component.selections_changed.emit(rects)
        task = self.task_list_component.get_task(index)
        self.append_output(tr['TaskList']['PresetAppliedToTask'].format(
            preset_name, task.name if task else index))

    def on_region_preset_apply_multi(self, rows: list, preset_name: str, areas_str: str):
        """将指定预设应用到多个选中任务"""
        rects = self._parse_preset_areas(areas_str)
        if not rects:
            return
        cur_idx = self.task_list_component.get_current_task_index()
        for i in rows:
            self.task_list_component.update_task_option(i, TaskOptions.SUB_AREAS, list(rects))
            if i == cur_idx:
                self.video_display_component.set_selection_rects(rects)
                self.video_display_component.selections_changed.emit(rects)
        self._region_clipboard = list(rects)
        self.task_list_component.set_paste_enabled(True)
        self.append_output(tr['TaskList']['PresetAppliedToSelected'].format(preset_name, len(rows)))

    def on_region_preset_apply_all(self, preset_name, areas_str):
        """将指定预设应用到所有任务"""
        rects = self._parse_preset_areas(areas_str)
        if not rects:
            return
        count = 0
        for i in range(len(self.task_list_component.get_all_tasks())):
            self.task_list_component.update_task_option(i, TaskOptions.SUB_AREAS, list(rects))
            count += 1
        # 同步更新当前显示的视频预览
        cur_idx = self.task_list_component.get_current_task_index()
        ...
        self._region_clipboard = list(rects)
        self.task_list_component.set_paste_enabled(True)
        self.append_output(tr['TaskList']['PresetAppliedToAll'].format(preset_name, count))

    @staticmethod
    def _parse_preset_areas(areas_str) -> list:
        """将预设的 areas 字符串解析为 [(ymin,ymax,xmin,xmax), ...] 列表"""
        rects = []
        for area in areas_str.split(";"):
            try:
                ymin, ymax, xmin, xmax = map(float, area.split(","))
                rects.append((ymin, ymax, xmin, xmax))
            except ValueError:
                continue
        return rects
                    ...
                    while not self._stop_event.is_set():
                        try:
                            ...
                            for key in task_item.options:
                                ...
                                if key == TaskOptions.SRT_PATH.value and not value:
                                    continue
    ...
    def update_progress(self, progress_total, isFinished):
        try:
            # 用户浏览其他任务时，跳过滑块更新：
            #   1. self.frame_count 已被覆写为浏览任务的帧数 → pos 计算会出错
            #   2. 强制移动滑块会打断用户手动拖动
            if self._user_browsing_task_index < 0:
    ...
    def _start_preview_load(self, path):
        """Load video/image preview in a background thread to keep UI responsive."""
        self.video_path = path
        ...
        def worker():
            try:
                ...
                self.preview_loaded_signal.emit(path, data)
            except Exception as e:
    ...
    @Slot(str, object)
    def _on_preview_loaded(self, path, data):
        """Apply preview loaded in background thread (runs on main thread via signal)."""
        # Ignore stale result if user has navigated to a different file
        ...
        self.update_preview(data['frame'])
        self.video_slider.setMaximum(self.frame_count)
        self.video_slider.setValue(1)
        self.video_display_component.set_dragger_enabled(True)
    ...
    def open_file(self):
        files, _ = QtWidgets.QFileDialog.getOpenFileNames(
            ...
            "",
            "All Files (*.*);;Video Files (*.mp4 *.flv *.wmv *.avi *.mkv *.mov);;Image Files (*.jpg *.jpeg *.png *.bmp *.webp *.tiff);;Subtitle Files (*.srt)"
        )
        if not files:
            return

        media_files = [path for path in files if not self.is_srt_file(path)]
        srt_files = [path for path in files if self.is_srt_file(path)]

        # Add all tasks to the list immediately without loading any video,
        # so the UI never freezes regardless of how many files are selected.
        added_indices = []
        for path in media_files:
            self.task_list_component.add_task(path)
            idx = self.task_list_component.find_task_index_by_path(path)
            if idx >= 0:
                added_indices.append(idx)
                self.append_output(f"{tr['SubtitleExtractorGUI']['OpenVideoSuccess']}: {path}")

        # Select first file and load its preview asynchronously.
        # Skip preview load while a task is already processing to avoid
        # overwriting the active processing preview.
        if added_indices:
            first_idx = added_indices[0]
            self.task_list_component.select_task(first_idx)
            first_task = self.task_list_component.get_task(first_idx)
            if first_task and self.current_processing_task_index < 0:
                self._start_preview_load(first_task.path)

        if srt_files:
            target_indices = added_indices if added_indices else None
            self.attach_srt_files_to_tasks(srt_files, target_indices=target_indices, log_unmatched=True)
        elif added_indices:
            self.attach_local_matching_srt_files(added_indices)

    @staticmethod
    def is_srt_file(path):
        return os.path.splitext(path)[-1].lower() == ".srt"
    ...
    @staticmethod
    def _compact_stem(stem):
        return "".join(ch for ch in stem.lower() if ch.isalnum())

    def score_srt_match(self, video_path, srt_path):
        video_stem = self._file_stem(video_path)
        srt_stem = self._file_stem(srt_path)
        separators = ("_", "-", ".", " ")
        if srt_stem == video_stem:
            return 1000
        if any(srt_stem.startswith(video_stem + sep) for sep in separators):
            return 900

        compact_video = self._compact_stem(video_stem)
        compact_srt = self._compact_stem(srt_stem)
        if compact_srt == compact_video:
            return 850
        if compact_srt.startswith(compact_video) and len(compact_srt) - len(compact_video) <= 8:
            return 800
        return 0

    def attach_local_matching_srt_files(self, task_indices):
        srt_candidates = []
        seen = set()
        for index in task_indices:
            ...
            for name in names:
                ...
                if self.is_srt_file(path) and path not in seen:
                    seen.add(path)
                    srt_candidates.append(path)
        if srt_candidates:
            self.attach_srt_files_to_tasks(srt_candidates, target_indices=task_indices, log_unmatched=False)

    def attach_srt_files_to_tasks(self, srt_paths, target_indices=None, log_unmatched=True):
        all_tasks = self.task_list_component.get_all_tasks()
        if not all_tasks:
            self.append_output(tr['SubtitleExtractorGUI']['OpenVideoFirst'])
            return 0

        if target_indices is None:
            target_indices = list(range(len(all_tasks)))
        target_indices = [idx for idx in target_indices if self.task_list_component.get_task(idx)]
        unique_srt_paths = []
        seen = set()
        for path in srt_paths:
            if not self.is_srt_file(path) or path in seen:
                continue
            seen.add(path)
            unique_srt_paths.append(path)

        task_candidates = {}
        unmatched = []
        ambiguous = []
        for srt_path in unique_srt_paths:
            scored = []
            for index in target_indices:
                task = self.task_list_component.get_task(index)
                score = self.score_srt_match(task.path, srt_path)
                if score > 0:
                    scored.append((score, index))
            if not scored:
                unmatched.append(srt_path)
                continue
            scored.sort(reverse=True)
            best_score = scored[0][0]
            best_indices = [index for score, index in scored if score == best_score]
            if len(best_indices) > 1:
                ambiguous.append(srt_path)
                continue
            task_candidates.setdefault(best_indices[0], []).append((best_score, srt_path))

        attached_count = 0
        for index, candidates in task_candidates.items():
            candidates.sort(reverse=True)
            best_score = candidates[0][0]
            best_paths = [path for score, path in candidates if score == best_score]
            task = self.task_list_component.get_task(index)
            if len(best_paths) > 1:
                ambiguous.extend(best_paths)
                continue
            srt_path = best_paths[0]
            self.task_list_component.update_task_option(index, TaskOptions.SRT_PATH, srt_path)
            self.append_output(tr['SubtitleExtractorGUI']['SRTMatchedLog'].format(os.path.basename(task.path), os.path.basename(srt_path)))
            attached_count += 1

        if log_unmatched:
            for srt_path in unmatched:
                self.append_output(tr['SubtitleExtractorGUI']['SRTUnmatchedLog'].format(os.path.basename(srt_path)))
            for srt_path in ambiguous:
                self.append_output(tr['SubtitleExtractorGUI']['SRTAmbiguousLog'].format(os.path.basename(srt_path)))

        self.update_srt_button_state()
        return attached_count

    def update_srt_button_state(self, index=None):
        if index is None:
            index = self.task_list_component.get_current_task_index()
        srt_path = self.task_list_component.get_task_option(index, TaskOptions.SRT_PATH, "") if index >= 0 else ""
        if srt_path:
            name = os.path.basename(srt_path)
            if len(name) > 32:
                name = name[:14] + "..." + name[-15:]
            self.srt_button.setText(tr['SubtitleExtractorGUI']['SRTSelected'].format(name))
            self.srt_button.setToolTip(srt_path)
            self.clear_srt_button.setEnabled(True)
        else:
            self.srt_button.setText(tr['SubtitleExtractorGUI']['AddSRT'])
    ...
    def choose_srt_file(self):
        ...
        if not task:
            self.append_output(tr['SubtitleExtractorGUI']['OpenVideoFirst'])
            return
        start_dir = os.path.dirname(task.path)
        srt_paths, _ = QtWidgets.QFileDialog.getOpenFileNames(
            self,
            tr['SubtitleExtractorGUI']['AddSRT'],
            start_dir,
            tr['SubtitleExtractorGUI']['SRTFileFilter'],
        )
        if not srt_paths:
            return
        if len(srt_paths) == 1:
            ...
            self.task_list_component.update_task_option(index, TaskOptions.SRT_PATH, srt_path)
            self.update_srt_button_state(index)
            self.append_output(tr['SubtitleExtractorGUI']['SRTSelectedLog'].format(srt_path))
        else:
            self.attach_srt_files_to_tasks(srt_paths, log_unmatched=True)

    def clear_srt_file(self):
        index = self.task_list_component.get_current_task_index()
        task = self.task_list_component.get_task(index)
        if not task:
            self.append_output(tr['SubtitleExtractorGUI']['OpenVideoFirst'])
            return
        self.task_list_component.update_task_option(index, TaskOptions.SRT_PATH, "")
        self.update_srt_button_state(index)
        self.append_output(tr['SubtitleExtractorGUI']['SRTClearedLog'])
