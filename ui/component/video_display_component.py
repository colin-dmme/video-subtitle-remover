...
class VideoDisplayComponent(QWidget):
    ...
    # 预设信号
    preset_saved = Signal(str)    # 参数：preset name
    ...
    # ──────────────────────────────────────────────
    # 预设 (Saved Region Presets)
    # ──────────────────────────────────────────────

    def _parse_presets(self) -> list:
        """将 config.savedRegionPresets 解析为 [{'name': str, 'areas': str}, ...]"""
    ...
    def _save_presets(self, presets: list):
        """将预设列表持久化到 config.json"""
        from qfluentwidgets import qconfig as _qconfig
        parts = [f"{p['name']}:{p['areas']}" for p in presets]
        config.savedRegionPresets.value = "||".join(parts)
        _qconfig.save()

    def __rebuild_preset_menus(self):
        """每次右键菜单弹出前动态重建 Load/Delete 子菜单"""
        self.menu_load_preset.clear()
        self.menu_delete_preset.clear()
        presets = self._parse_presets()
        for p in presets:
            load_action = QAction(p["name"], self)
            load_action.triggered.connect(
                lambda checked=False, preset=p: self.__handle_load_preset(preset))
            self.menu_load_preset.addAction(load_action)

            del_action = QAction(p["name"], self)
            del_action.triggered.connect(
                lambda checked=False, preset=p: self.__handle_delete_preset(preset))
            self.menu_delete_preset.addAction(del_action)

        self.menu_load_preset.setEnabled(bool(presets))
        self.menu_delete_preset.setEnabled(bool(presets))

    def __handle_save_preset(self):
        """保存当前选区为命名预设"""
        ...
        name, ok = QInputDialog.getText(
            self,
            tr['SubtitleExtractorGUI']['SaveRegionPreset'],
            tr['SubtitleExtractorGUI']['RegionPresetNameTitle'])
        if not ok:
            return
        name = name.strip()
        if not name:
        ...
        areas_str = ";".join(
            f"{round(r[0],4)},{round(r[1],4)},{round(r[2],4)},{round(r[3],4)}"
        ...
        presets.append({"name": name, "areas": areas_str})
        presets = presets[-20:]  # 最多保留 20 个预设
        self._save_presets(presets)
        self.preset_saved.emit(name)

    def __handle_load_preset(self, preset: dict):
        """从预设恢复选区"""
        rects = []
        for area in preset["areas"].split(";"):
            try:
        ...
        if rects:
            ...
            self.selections_changed.emit(rects)
            self.preset_loaded.emit(preset["name"])

    def __handle_delete_preset(self, preset: dict):
        """删除指定预设"""
        presets = [p for p in self._parse_presets() if p["name"] != preset["name"]]
        self._save_presets(presets)
        self.preset_deleted.emit(preset["name"])
    ...
    def __init_context_menu(self):
        ...
        self.action_save_preset = QAction(tr['SubtitleExtractorGUI']['SaveRegionPreset'], self)
        self.action_save_preset.triggered.connect(self.__handle_save_preset)
        self.context_menu.addAction(self.action_save_preset)

        self.menu_load_preset = QMenu(tr['SubtitleExtractorGUI']['LoadRegionPreset'], self)
        self.context_menu.addMenu(self.menu_load_preset)

        self.menu_delete_preset = QMenu(tr['SubtitleExtractorGUI']['DeleteRegionPreset'], self)
        self.context_menu.addMenu(self.menu_delete_preset)

        # 每次弹出菜单前动态重建预设子菜单
        self.context_menu.aboutToShow.connect(self.__rebuild_preset_menus)
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            self.context_menu.aboutToShow.disconnect(self.__rebuild_preset_menus)
            self.action_save_preset.triggered.disconnect(self.__handle_save_preset)
