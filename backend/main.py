...
from backend.tools.srt_timeline import (
    intersect_frame_sections,
    intervals_to_frame_spans,
    parse_srt_intervals,
    spans_to_ranges,
)
...
class SubtitleRemover:
    def __init__(self, vd_path, gui_mode=False):
        ...
        # Optional external SRT timeline. If provided, it is converted to ab_sections before processing.
        self.subtitle_srt_path = None
        self._srt_timeline_applied = False
    ...
    def propainter_mode(self, tbar):
        ...
        last_failed = 0  # tracks reader.failed_count to distinguish EOF vs corrupt frame
        while True:
            ret, frame = reader.read()
            if not ret:
                if reader.failed_count > last_failed:
                    # Corrupt frame — skip, advance index, update progress, continue
                ...
                if self.is_current_frame_no_start(index, continuous_frame_no_list):
                    ...
                    if end_frame_no != -1:
                        ...
                        while index < end_frame_no:
                            ...
                            if not ret:
                                if reader.failed_count > last_failed:
                                    # Corrupt frame inside batch — skip, advance index.
                                    # last_failed is intentionally shared with the outer loop
                                    # so outer-loop discrimination still works after this batch.
                                    last_failed = reader.failed_count
                                    index += 1
                                    continue
                                break  # True EOF inside batch
                            index += 1
                            temp_frames.append(frame)
                        # ************ 读取该区间所有帧 end ************
        ...
        # --- warn user if any frames were skipped due to read errors ---
        if reader.failed_count > 0:
            preview = ', '.join(str(x) for x in reader.failed_frames[:10])
    ...
    def apply_srt_timeline_if_needed(self):
        if not self.subtitle_srt_path:
            return
        if self.is_picture:
            self.append_output(tr['Main']['SRTTimelineIgnoredForImage'].format(self.subtitle_srt_path))
            return
        if not os.path.exists(self.subtitle_srt_path):
            raise Exception(tr['Main']['ReadFileFailed'].format(self.subtitle_srt_path))

        intervals = parse_srt_intervals(self.subtitle_srt_path)
        if not intervals:
            raise Exception(tr['Main']['SRTTimelineEmpty'].format(self.subtitle_srt_path))

        backward_frames = config.subtitleTimelineBackwardFrameCount.value
        forward_frames = config.subtitleTimelineForwardFrameCount.value
        srt_spans = intervals_to_frame_spans(
            intervals,
            self.fps,
            self.frame_count,
            backward_frames,
        ...
        srt_sections = spans_to_ranges(srt_spans)
        if not srt_sections:
            raise Exception(tr['Main']['SRTTimelineEmpty'].format(self.subtitle_srt_path))

        if self.ab_sections is not None and len(self.ab_sections) > 0:
            srt_sections = intersect_frame_sections(self.ab_sections, srt_sections, self.frame_count)
            if not srt_sections:
                raise Exception(tr['Main']['SRTTimelineIntersectedEmpty'].format(self.subtitle_srt_path))

        self.ab_sections = srt_sections
        self._srt_timeline_applied = True
        self.append_output(tr['Main']['SRTTimelineLoaded'].format(
            self.subtitle_srt_path,
            len(srt_sections),
            backward_frames,
            forward_frames,
        ))
    ...
    def run(self):
        ...
        self.apply_srt_timeline_if_needed()
...
if __name__ == '__main__':
    ...
    sr.subtitle_srt_path = args.subtitle_srt
    if args.srt_padding_before is not None:
        config.subtitleTimelineBackwardFrameCount.value = args.srt_padding_before
    if args.srt_padding_after is not None:
        config.subtitleTimelineForwardFrameCount.value = args.srt_padding_after
