...
class STTNAutoInpaint:
    ...
    def __call__(self, input_mask=None, input_sub_remover=None, tbar=None):
        ...
        try:
            ...
            last_sttn_failed = 0  # tracks prefetcher.failed_count to distinguish EOF vs corrupt frame
            # 遍历每一次的迭代次数
            for i in range(rec_time):
                ...
                for j in range(start_f, end_f):
                    ...
                    if not success:
                        if prefetcher.failed_count > last_sttn_failed:
                            # Corrupt frame — warn user, skip, continue to next frame in segment.
                            # last_sttn_failed shared across segments so outer summary stays accurate.
                            last_sttn_failed = prefetcher.failed_count
                            ...
                            if input_sub_remover is not None:
                                ...
                                print(warn_msg)
                            continue
                        break  # True EOF — stop reading this segment
                ...
                if valid_frames_count == 0:
                    warn_msg = f"⚠ Không có khung hình hợp lệ trong đoạn {start_f+1}-{end_f}. Bỏ qua đoạn này."
                    if input_sub_remover is not None:
                        input_sub_remover.append_output(warn_msg)
                    else:
                        print(warn_msg)
            ...
            if prefetcher.failed_count > 0:
                preview = ', '.join(str(x) for x in prefetcher.failed_frames[:10])
                suffix = ' ...' if prefetcher.failed_count > 10 else ''
                summary_msg = (
                    f"⚠ Tổng cộng {prefetcher.failed_count} khung hình lỗi đã bị bỏ qua. "
                    f"Đầu ra có thể bị thiếu tại khung: {preview}{suffix}"
                )
                if input_sub_remover is not None:
                    input_sub_remover.append_output(summary_msg)
                else:
                    print(summary_msg)
